...
Angel Fallen 已经从早期房间制 Roguelike 骨架推进到“可验证的类幸存者核心 v1”。当前自动化基线为：Godot 无头启动通过，JSON 语法与配置语义校验通过；2026-05-07 的 Stage 8.1 第一轮已完成资产试点清单、程序化 fallback 候选、第一批 catalog / `.tres` preview 连接，以及 `auto_weapon.gd` 的 weapon runtime profile resolver 行为等价拆分。
...
| Stage 8.1 资产试点 | 已冻结第一批角色/敌人/武器/UI/SFX 清单；无 API 密钥时已落地程序化 fallback candidate / v1，并连接 8 个资源 preview | `spec-design/stage8-asset-pilot-implementation-plan-2026-05-07.md`、`data/pipelines/stage8_asset_pilot_batch_manifest.json`、`resources/resource_catalog.json` |
| Cleanup-3 武器 profile 拆分 | 已把武器运行时 profile、范围、颜色、环绕/光环映射抽到 resolver，AutoWeapon 保留发射执行职责 | `scripts/systems/weapon_runtime_profile_resolver.gd`、`test/unit/test_weapon_runtime_profile_resolver.gd` |
...
| 优先级 | 缺口 | 说明 |
|--------|------|------|
| P0 | 正式资产替换 | Stage 8.1 已有 fallback 候选和 preview 连接，但正式 AI/人工成品资产、运行时 sprite / SFX 全量替换、MCP 截图验收仍是发布前主阻塞。 |
| P0 | 大文件拆分 | `auto_weapon.gd` 已完成第一轮 resolver 拆分；`game_world.gd`、`main_menu.gd`、`run_visual_snapshot_regression.gd` 仍是高风险文件，继续堆功能会降低稳定性。 |
...
- `auto_weapon.gd`：保留主执行器，不同武器族的 runtime profile、范围、颜色、环绕/光环映射已在 2026-05-07 迁入 `scripts/systems/weapon_runtime_profile_resolver.gd`；后续只继续抽地面区、贴图解析和统计，不混入新玩法。
...
1. 补齐 Stage 8.1 正式资产来源：配置 API 密钥或导入人工筛选图像/音频，把 fallback candidate 替换为正式 candidate。
2. 做运行时接入第二轮：角色/敌人 concept 显示、武器 projectile/impact 贴图、拾取/击杀/命中 SFX cue，并保存 Godot MCP 截图。
3. 推进 UI 视觉统一：主菜单、设置、HUD、构筑面板、宝箱、死亡结算在 720p/1080p/4K 下统一字体层级、按钮态和面板密度。
4. 继续 Cleanup-1/2：先抽 `game_world.gd` 奖励/宝箱/拾取结算边界，再拆 `main_menu.gd` 的 run setup、archive/codex、settings 子面板逻辑。
5. 做 5/10/20 分钟平衡 probe 与 Godot MCP 人工截图复核，调敌潮、升级、宝箱、掉落和伤害曲线。
6. 最后再进入发布门禁：导出包、平台 smoke、资源验收、兼容性 rehearsal。
