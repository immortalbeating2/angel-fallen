...
| [stage8-asset-pilot-implementation-plan-2026-05-07.md](./stage8-asset-pilot-implementation-plan-2026-05-07.md) | Stage 8.1 资产替换试点执行计划：冻结第一批角色、敌人、武器、UI 与 SFX 候选路径、生成配置、接入顺序和 MCP 证据目录 |
...
| 0.5.02 | 2026-05-07 | 新增 Stage 8.1 资产替换试点执行计划与首批批量生成配置，冻结第一批候选路径、接入顺序和 MCP 证据目录 |
| 0.5.03 | 2026-05-07 | Stage 8.1 第一批候选落地：无 API 密钥时生成程序化 fallback candidate / v1，并输出试点 manifest |
| 0.5.04 | 2026-05-07 | Stage 8.1 第一批 preview 接入：连接角色、敌人、武器 catalog / `.tres` 预览路径，并保留 `wpn_solar_lance` catalog gap |
| 0.5.05 | 2026-05-07 | Cleanup-3 第一轮：抽出 weapon runtime profile resolver，降低 `auto_weapon.gd` 中数值映射和发射执行耦合 |
