...
func test_map_generator_challenge_layer_runtime_plans_cover_cl1_cl2_cl3_and_cl4() -> void:
	var cfg := _load_map_config()
	var generator_script: Script = load("res://scripts/systems/map_generator.gd")
	assert_not_null(generator_script, "map_generator.gd should be loadable")
	if generator_script == null:
	...
	var generator: Node = generator_script.new()
	var cl1_plan: Dictionary = generator.generate_challenge_layer_run_plan("CL1", cfg)
	var cl2_plan: Dictionary = generator.generate_challenge_layer_run_plan("CL2", cfg)
	var cl3_plan: Dictionary = generator.generate_challenge_layer_run_plan("CL3", cfg)
	var cl4_plan: Dictionary = generator.generate_challenge_layer_run_plan("CL4", cfg)
	generator.free()

	assert_eq(int(cl1_plan.get("room_count", 0)), 3, "CL1 runtime plan should keep the entry/combat/settlement shell")
	assert_eq(str(cl1_plan.get("room_count_label", "")), "Entry + combat + settlement", "CL1 runtime plan should preserve the stage-6 baseline route frame")
	var cl1_rooms: Array = cl1_plan.get("rooms", [])
	assert_eq(cl1_rooms.size(), 3, "CL1 runtime plan should keep three playable rooms")
	assert_eq(str((cl1_rooms[1] as Dictionary).get("room_type", "")), "combat", "CL1 middle room should remain a combat room")
	assert_eq(str((cl1_rooms[2] as Dictionary).get("challenge_phase", "")), "settlement", "CL1 final room should remain the settlement phase")

	assert_eq(int(cl2_plan.get("start_room", 0)), 1, "CL2 runtime plan should start at room 1")
	assert_eq(int(cl2_plan.get("room_count", 0)), 4, "CL2 runtime plan should expose entry/elite/boss/settlement rooms")
	assert_eq(str(cl2_plan.get("room_count_label", "")), "Entry + elite + boss + settlement", "CL2 runtime plan should advertise the deeper route frame")
	var cl2_rooms: Array = cl2_plan.get("rooms", [])
	assert_eq(cl2_rooms.size(), 4, "CL2 runtime plan should build four playable rooms")
	assert_eq(str((cl2_rooms[0] as Dictionary).get("room_type", "")), "safe_camp", "CL2 should open with a staging room")
	assert_eq(str((cl2_rooms[1] as Dictionary).get("room_type", "")), "elite", "CL2 second room should be an elite trial")
	assert_eq(str((cl2_rooms[2] as Dictionary).get("room_type", "")), "boss", "CL2 should include a boss phase before settlement")
	assert_eq(str((cl2_rooms[2] as Dictionary).get("title", "")), "Crown Trial", "CL2 boss room should expose the crown-trial title")
	assert_eq(str((cl2_rooms[3] as Dictionary).get("room_type", "")), "safe_camp", "CL2 should end in a settlement room")
	assert_eq(str((cl2_rooms[3] as Dictionary).get("challenge_layer_id", "")), "CL2", "CL2 settlement should stay tagged to the second challenge layer")

	assert_eq(int(cl3_plan.get("start_room", 0)), 1, "CL3 runtime plan should start at room 1")
	assert_eq(int(cl3_plan.get("room_count", 0)), 5, "CL3 runtime plan should expose entry/elite/combat/boss/settlement rooms")
	assert_eq(str(cl3_plan.get("room_count_label", "")), "Entry + elite + combat + boss + settlement", "CL3 runtime plan should advertise the sovereign route frame")
	var cl3_rooms: Array = cl3_plan.get("rooms", [])
	assert_eq(cl3_rooms.size(), 5, "CL3 runtime plan should build five playable rooms")
	assert_eq(str((cl3_rooms[0] as Dictionary).get("room_type", "")), "safe_camp", "CL3 should open with a staging room")
	assert_eq(str((cl3_rooms[1] as Dictionary).get("room_type", "")), "elite", "CL3 second room should be an elite gauntlet")
	assert_eq(str((cl3_rooms[2] as Dictionary).get("room_type", "")), "combat", "CL3 third room should add a sustained combat breach")
	assert_eq(str((cl3_rooms[3] as Dictionary).get("room_type", "")), "boss", "CL3 should include a sovereign boss phase before settlement")
	assert_eq(str((cl3_rooms[3] as Dictionary).get("title", "")), "Sovereign Echo", "CL3 boss room should expose the sovereign-echo title")
	assert_eq(str((cl3_rooms[4] as Dictionary).get("room_type", "")), "safe_camp", "CL3 should end in a settlement room")
	assert_eq(str((cl3_rooms[4] as Dictionary).get("challenge_layer_id", "")), "CL3", "CL3 settlement should stay tagged to the third challenge layer")

	assert_eq(int(cl4_plan.get("start_room", 0)), 1, "CL4 runtime plan should start at room 1")
	assert_eq(int(cl4_plan.get("room_count", 0)), 6, "CL4 runtime plan should expose entry/elite/combat/elite/boss/settlement rooms")
	assert_eq(str(cl4_plan.get("room_count_label", "")), "Entry + elite + combat + elite + boss + settlement", "CL4 runtime plan should advertise the apex route frame")
	var cl4_rooms: Array = cl4_plan.get("rooms", [])
	assert_eq(cl4_rooms.size(), 6, "CL4 runtime plan should build six playable rooms")
	assert_eq(str((cl4_rooms[0] as Dictionary).get("room_type", "")), "safe_camp", "CL4 should open with a staging room")
	assert_eq(str((cl4_rooms[1] as Dictionary).get("room_type", "")), "elite", "CL4 second room should be an elite ascent room")
	assert_eq(str((cl4_rooms[2] as Dictionary).get("room_type", "")), "combat", "CL4 third room should be a sustained combat breach")
	assert_eq(str((cl4_rooms[3] as Dictionary).get("room_type", "")), "elite", "CL4 fourth room should add a second elite gate")
	assert_eq(str((cl4_rooms[4] as Dictionary).get("room_type", "")), "boss", "CL4 should include an apex boss phase before settlement")
	assert_eq(str((cl4_rooms[4] as Dictionary).get("title", "")), "Apex Throne", "CL4 boss room should expose the apex-throne title")
	assert_eq(str((cl4_rooms[5] as Dictionary).get("room_type", "")), "safe_camp", "CL4 should end in a settlement room")
	assert_eq(str((cl4_rooms[5] as Dictionary).get("challenge_layer_id", "")), "CL4", "CL4 settlement should stay tagged to the fourth challenge layer")
